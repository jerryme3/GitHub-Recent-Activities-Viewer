Steps on how to use this:

1. Clone the repository anywhere in your computer
2. Make sure that you have JDK installed in your computer, since this is written in Java
3. Open the terminal inside the copied repository folder
4. Paste this command to your terminal: javac github_activity.java 
5. Then, paste this command after the other one: java github_activity <username-to-search> 
